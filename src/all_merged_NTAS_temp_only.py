# %%
# Script to create a CF-compliant NetCDF file from the merged NTAS dataset
...
input_file = os.path.join(merged_dir, 'merged_NTAS11_to_NTAS20.nc')
output_dir = '/Users/Shared/ORS/DEEP_TS/NTAS/merged_NTAS'
output_file = os.path.join(output_dir, 'merged_NTAS_2011_to_2022.nc')
temp_file = os.path.join(output_dir, 'NTAS_temperature_2011_to_2022_v1.nc')
...
if not lat_str or not lon_str:
    ...
    # Use default NTAS location if missing
    lat_str = '-20.0'  
    lon_str = '-85.5'
...
# Ensure variables declare coordinates via attributes (Dataset holds actual coords)
for var_name in list(cf_ds.data_vars):
    if var_name not in ['latitude', 'longitude', 'depth', 'time']:
...
cf_ds.attrs['principal_investigator'] = 'Albert Plueddemann'
cf_ds.attrs['principal_investigator_email'] = 'aplueddemann@whoi.edu'
cf_ds.attrs['institution'] = 'Woods Hole Oceanographic Institution'
cf_ds.attrs['experiment'] = 'NTAS Ocean Reference Station'
cf_ds.attrs['site_code'] = 'NTAS'
cf_ds.attrs['platform_code'] = 'NTAS'
cf_ds.attrs['wmo_platform_code'] = '48401'

# Add funding acknowledgment
cf_ds.attrs['acknowledgement'] = 'The NTAS project is supported by the National Oceanic and Atmospheric Administration (NOAA) Global Ocean Monitoring and Observing (GOMO) Program through the Cooperative Institute for the North Atlantic Region (CINAR) under Cooperative Agreement NA14OAR4320158. NOAA CPO FundRef number (100007298).'
...
temp_only_ds.attrs['title'] = 'NTAS Ocean Reference Station - Temperature Only'
temp_only_ds.attrs['summary'] = 'Sea water temperature measurements from NTAS moorings 11-20 (2011-2022)'
...
temp_output_file = os.path.join(output_dir, 'NTAS_temperature_2011_to_2022_v1.nc')
