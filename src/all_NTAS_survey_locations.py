# plot distance from NTAS 12 and distance from previous site
import xarray as xr
...
def plot_deployment_locations():
    """
    Create cumulative plots of NTAS mooring deployment locations.
    Each figure shows locations from NTAS 12 up to the specified deployment.
    Also calculates distance between consecutive deployments.
    """
    # Load the merged dataset
    merged_data_dir = '/Users/Shared/ORS/DEEP_TS/NTAS/merged_NTAS'
    ds = xr.open_dataset(f'{merged_data_dir}/merged_NTAS11_to_NTAS20.nc')
    ...
    for i in range(2, len(deployments) + 1):  # Start with at least 2 deployments
        ...
        for j in range(i):
            ...
            legend_handles.append(
                Line2D(
                    ...
                    markeredgecolor=high_contrast_colors[color_idx],
                    markeredgewidth=1.8,
                )
            )
            legend_labels.append(f'NTAS {deployment_num}')
        ...
        ax.set_title(f'NTAS Deployment Locations (NTAS {start_deployment} to {end_deployment})', 
        ...
        ax.legend(legend_handles, legend_labels, loc='lower right', title='Deployments', 
                ncol=2, fontsize=14, title_fontsize=16)  # Increased font sizes
        
        # Save figure
        plt.tight_layout()
        plt.savefig(f'{img_dir}/NTAS{start_deployment}_to_NTAS{end_deployment}_locations.png', 
                   dpi=300, bbox_inches='tight')
        print(f"Created map for NTAS {start_deployment} to {end_deployment}")
        plt.close()
        ...
        if i >= 2:  # Changed from i >= 3
            ...
            doc_dir = f'/Users/yugao/UOP/ORS-processing/doc/NTAS/{last_deployment}'
            ...
            with open(f'{doc_dir}/deployment_distance.tex', 'w') as f:
                f.write("% NTAS mooring deployment distance information\n\n")
                
                # Simple sentence about the distance
                f.write(f"The distance between NTAS {second_last_deployment} and NTAS {last_deployment} ")
                ...
                f.write("\\hline\n")
                f.write(f"NTAS {second_last_deployment} & {lat1_str} & {lon1_str} \\\\\n")
                f.write(f"NTAS {last_deployment} & {lat2_str} & {lon2_str} \\\\\n")
                f.write("\\hline\n")
                f.write("\\end{tabular}\n")
                f.write("\\end{table}\n")
