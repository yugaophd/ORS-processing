...
# from mooring log
water_depth_from_ship_uncorrected = 4541     # uncorrected water depth, depth recorder reading
...
# %%
# spike times
# SBE16	1873	deep	1800 sec	28-Feb-14	13:00	2/28/14 13:27	2/28/14 14:21
# SBE16	1875	deep	1800 sec	28-Feb-14	13:00	2/28/14 13:27	2/28/14 14:21
deployment_spike_times_start = pd.to_datetime('2/28/2014 13:00')
deployment_spike_times_end = pd.to_datetime('2/28/2014 14:21') # matching the diagram

# SBE16	1873	    17:00:00	20150426	18:40	20150426
# SBE16	1875		17:00:00	20150426	18:40	20150426

recovery_spike_times_start = pd.to_datetime('04/26/2015 17:00')
recovery_spike_times_end = pd.to_datetime('04/26/2015 18:40') # matching the diagram


# %%
# file input
mat_file_path1 = f'/Users/Shared/ORS/Stratus/{case_name}/data/sbe16/{case_name}_sbe16_1876.mat'
mat_file_path2 = f'/Users/Shared/ORS/Stratus/{case_name}/data/sbe16/{case_name}_sbe16_1879.mat'
...
release_fired_time = pd.to_datetime(ds.attrs['platform_buoy_recovery_time'])
# 
anchor_over_time2 =   pd.to_datetime(ds2.attrs['platform_anchor_over_time'])
release_fired_time2 = pd.to_datetime(ds2.attrs['platform_buoy_recovery_time'])
...
from util import fill_or_create_variables

# # The above code is using the `fill` method to fill the string with `#` characters until it reaches
...
valid_time_window2.attrs['water_depth_from_ship_uncorrected_m'] = water_depth_from_ship_uncorrected 
valid_time_window.attrs['instrument_depth'] = water_depth_from_ship_corrected - instrument_height_above_bottom
...
valid_time_window2.attrs['water_depth_from_ship_corrected_m'] = water_depth_from_ship_corrected 
...
valid_time_window['sea_water_practical_salinity'].attrs['comment'] = f'Pratical salinity derived from TEOS-10 sw_sal calculation at a fixed depth of {mat_data1['depth']} m.'
valid_time_window['sea_water_absolute_salinity'].attrs['comment'] = f'Absolute salinity derived from practical salinity measurements at {mat_data1["depth"]} m depth using the TEOS-10 sw_sal conversion algorithm'
valid_time_window['sea_water_pressure'].attrs['comment'] = 'Pressure comes from instrument without any further processing.'

valid_time_window2['sea_water_practical_salinity'].attrs['comment'] = f'Derived from TEOS-10 sw_sal calculation at a fixed depth of {mat_data1['depth']} m.'
valid_time_window2['sea_water_absolute_salinity'].attrs['comment'] = f'Absolute salinity derived from practical salinity measurements at {mat_data2["depth"]} m depth using the TEOS-10 sw_sal conversion algorithm'
valid_time_window2['sea_water_pressure'].attrs['comment'] = 'Pressure comes from instrument without any further processing.'
...
extension_time = pd.Timedelta(hours=2)
deployment_spike_data = ds.sel(time=slice(deployment_spike_times_start - extension_time, 
                                deployment_spike_times_end + extension_time))

recovery_spike_data = ds.sel(time=slice(recovery_spike_times_start - extension_time, 
                            recovery_spike_times_end + extension_time))
...
labels = ['Temperature (°C)', 
        ...
        'Conductivity (S/m)', 
        'Pressure']
panel_titles = ['Temperature Profile', 'Salinity Measurements', 
                'Absolute Salinity', 
                'Conductivity Levels', 'Pressure Profile']
