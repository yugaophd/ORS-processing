...
# %%
# file input
mat_file_path1 = f'/Users/Shared/ORS/Stratus/{case_name}/data/sbe16/s13_sbe16_1873.mat'
mat_file_path2 = f'/Users/Shared/ORS/Stratus/{case_name}/data/sbe16/s13_sbe16_1875.mat'
...
# %%
    ...
    try:
        ...
        ds1_deploy_phase = ds1.sel(time=slice(anchor_over_time1 - pd.Timedelta(hours=6), 
                                           anchor_over_time1 + pd.Timedelta(hours=4)))
        ds1_recovery_phase = ds1.sel(time=slice(release_fired_time1 - pd.Timedelta(hours=6), 
                                             release_fired_time1 + pd.Timedelta(hours=4)))
        
        # Plot first instrument deployment/recovery phases
        phase_img_path1 = os.path.join(img_path, f"{case_name}_{sn1}_deployment_recovery_{version}.png")
        plot_spike_data(ds1_deploy_phase, ds1_recovery_phase, case_name, phase_img_path1,
                      deployment_time=anchor_over_time1, recovery_time=release_fired_time1)
        print(f"Deployment and recovery phase plot saved as {phase_img_path1}")
        
        # Process second instrument deployment/recovery phases
        ds2_deploy_phase = ds2.sel(time=slice(anchor_over_time2 - pd.Timedelta(hours=6), 
                                           anchor_over_time2 + pd.Timedelta(hours=4)))
        ds2_recovery_phase = ds2.sel(time=slice(release_fired_time2 - pd.Timedelta(hours=6), 
                                             release_fired_time2 + pd.Timedelta(hours=4)))
        
        # Plot second instrument deployment/recovery phases
        phase_img_path2 = os.path.join(img_path, f"{case_name}_{sn2}_deployment_recovery_{version}.png")
        plot_spike_data(ds2_deploy_phase, ds2_recovery_phase, case_name, phase_img_path2,
                      deployment_time=anchor_over_time2, recovery_time=release_fired_time2)
    ...
    print("Need at least two processed datasets to create comparison plots")
