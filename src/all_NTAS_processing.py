# %%
# NTAS deployment processing script
...
parser = argparse.ArgumentParser(description='Process one NTAS deployment config')
parser.add_argument('--config', default='NTAS20_config.json', help='Path to deployment config JSON')
...
metadata_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "NTAS_metadata.json")
try:
...
for instrument in config['instruments']:
    ...
    # Add water depth information - handle None values with descriptive text
    water_depth_uncorrected = water_depth_from_ship_uncorrected
    if water_depth_uncorrected is None or water_depth_uncorrected == -999:
        water_depth_uncorrected = "missing"

    valid_time_window.attrs['water_depth_from_ship_uncorrected_m'] = water_depth_uncorrected
    ...
    time_encoding = {'time': {'dtype': 'float64'}}
    valid_time_window.to_netcdf(output_file, engine='netcdf4', encoding=time_encoding)
