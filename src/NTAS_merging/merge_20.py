...
# Chose NTAS19-12246 and NTAS20-11392 for merging
...
project_name = 'NTAS'
project_number = 19 # the project number of the last merged dataset

# store the results in the doc folder of the latter case
case_name0 = f'{project_name}{project_number}'
instrument0_1 = '12246' # last merged dataset NTAS14-11392

case_name1 = f'{project_name}{project_number+1}'
instrument1_1 = '11392' #chosen for merging
instrument1_2 = '11393'
...
# %%
# resample/average NTAS 16
# NTAS 13 and 16 have different sampling frequency
# so we need to resample/average NTAS 16 to assess the difference

# Upsample NTAS 16 using linear interpolation
ds1_resampled1 = ds1_instrument1.resample(time='30min').mean()
ds1_resampled2 = ds1_instrument2.resample(time='30min').mean()
ds1_resampled1['time'] = ds1_resampled1.time.dt.round('30min')
ds1_resampled2['time'] = ds1_resampled2.time.dt.round('30min')
...
if overlap_start >= overlap_end:
    ...
    # Make time labels unique (cheap & robust), then align on overlap
    sel0_1 = sel0_1.groupby("time").first()
    sel1_1 = sel1_1.groupby("time").first()
    sel1_2 = sel1_2.groupby("time").first()

    sel0_1, sel1_1 = xr.align(sel0_1, sel1_1, join="inner", copy=False)
    _,      sel1_2 = xr.align(sel0_1, sel1_2, join="inner", copy=False)
    ...
    # Ensure unique, sorted time on both Datasets (cheap + robust)
    # ds0_instrument1 = ds0_instrument1.groupby("time").first().sortby("time")
    # ds1_resampled1  = ds1_resampled1.groupby("time").first().sortby("time")

    # Now safe to align & subtract
    # ds0_instrument1, ds1_resampled1 = xr.align(ds0_instrument1, ds1_resampled1, join="inner", copy=False)
    difference =  sel0_1 - sel1_1 #ds0_instrument1[variable] - ds1_resampled1[variable]
...
# merged_dataset.attrs['notes'] = f"NTAS 15 deep temperature data is missing because of instrument failure. We merged NTAS 11 to NTAS 14 with NTAS 16 at {merge_point} based on minimal differences, and use _FillValue to fill the missing data."
...
#%%
# revise specific attributes and save the merged dataset
merged_dataset.attrs['principal_investigator_email'] = 'aplueddemann@whoi.edu'
merged_dataset.attrs['institution'] = 'Woods Hole Oceanographic Institution'
merged_dataset.attrs['experiment'] = 'NTAS Ocean Reference Station'
merged_dataset.attrs['site_code'] = 'NTAS'
merged_dataset.attrs['platform_code'] = 'NTAS'
merged_dataset.attrs['wmo_platform_code'] = '48401'
